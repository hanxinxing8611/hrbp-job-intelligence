...
function loadData() {
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      console.log(`已加载 ${jobs.length} 条职位数据，${companies.length} 家公司`);
      return true;
    }
  } catch (e) {
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/api/status') {
    ...
    res.end(JSON.stringify({
      ...
      cityStats: Object.entries(cityStats).map(([city, count]) => ({ city, count })).sort((a, b) => b.count - a.count).slice(0, 15),
